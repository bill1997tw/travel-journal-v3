...
function isSuspiciousAutofillAddress(value) {
  const text = String(value || "").trim();
  if (!text) return true;
...
function sanitizeAutofillAddress(value) {
  const text = String(value || "").trim();
  return isSuspiciousAutofillAddress(text) ? "" : text;
}
  ...
  const requestPromise = (async () => {
    try {
      ...
      try {
        ...
        const normalized = normalizeGooglePlaceResponse(await response.json());
        if (normalized) {
          normalized.address = sanitizeAutofillAddress(normalized.address);
        }
        return normalized;
      } finally {
